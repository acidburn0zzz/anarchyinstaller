  ...
  while (true); do # Begin user menu while loop
    ...
    user="$(dialog --extra-button --extra-label "${edit}" --ok-button "${new_user}" --cancel-button "${done_msg}" --menu "${user_menu_msg}" 13 55 4 \
      "$(while IFS= read -r i; do
        echo "${i} $(cut <"${tmp_passwd}" -d: -f1,2 | grep -w "${i}" | cut -d: -f2)"
...
add_user() {
  if "${menu_enter}"; then
    ...
    input="$(echo "${pass_crypt}" | openssl enc -aes-256-cbc -a -d -salt -pbkdf2 -iter 1000 -pass pass:"${ssl_key}")"
    (printf "%s\n%s" "${input}" "${input}" | arch-chroot "${ARCH}" passwd "${user}") &>/dev/null &
    pid=$! pri=0.1 msg="${wait_load} \n\n \Z1> \Z2passwd ${user}\Zn" load
    unset input
    ...
    while IFS= read -r i; do
      ...
      (printf "%s\n%s" "${input}" "${input}" | arch-chroot "${ARCH}" passwd "$(cut <<<"${i}" -d: -f1)") &>/dev/null &
      pid=$! pri=0.1 msg="${wait_load} \n\n \Z1> \Z2passwd $(cut <<<"${i}" -d: -f1)\Zn" load
      unset input
      echo "$(date -u "+%F %H:%M") : Password set: $(cut <<<"${i}" -d: -f1)" >>"${log}"
