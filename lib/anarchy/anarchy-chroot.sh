...
anarchy_chroot() {
	...
	while (true); do
		echo -n "<${Red}root${Yellow}@${Green}${hostname}-chroot${Yellow}>:" \
			"${working_dir}>${Red}# ${ColorOff}"
		while IFS= read -r -n 1 -s char; do
			...
			if [ "${char}" = $'\x1b[D' ]; then
				...
				input="${input%?}"
				echo -ne "\r\033[K${Yellow}<${Red}root${Yellow}@${Green}${hostname}" \
					"-chroot${Yellow}>: ${working_dir}>${Red}# ${ColorOff}${input}"
			# User input up
				...
				if [ ${histindex} -gt 0 ]; then
					...
					echo -ne "\r\033[K${Yellow}<${Red}root${Yellow}@${Green}${hostname}" \
						"-chroot${Yellow}>: ${working_dir}>${Red}#" \
						"${ColorOff}${history[${histindex}]}"
				fi
			# User input down
		...
		if [ "${input}" = 'anarchy' ] || [ "${input}" = 'exit' ]; then
			...
			clear
			break
		elif (grep <<<"${input}" "^cd " &>/dev/null); then
			ch_dir="$(cut <<<"${input}" -c4-)"
			arch-chroot "${ARCH}" /bin/bash -c "cd ${working_dir}; cd ${ch_dir};" \
				"pwd > /etc/chroot_dir.var"
			mv "${ARCH}"/etc/chroot_dir.var /tmp/
			working_dir="$(</tmp/chroot_dir.var)"
		elif (grep <<<"${input}" "^help" &>/dev/null); then
